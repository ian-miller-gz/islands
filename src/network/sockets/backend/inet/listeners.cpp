...
static auto bound(
  NETWORK::Descriptor descriptor, const NETWORK::Socket &socket,
  Whole backlog) -> Flag {
  int yes = 1;
  setsockopt(
...
auto NETWORK::LISTENERS::create(const Socket &socket, Whole backlog) -> Handle {
  ...
  if (descriptor == CLOSED || !bound(descriptor, socket, backlog)) {
    logger.debug("Cannot listen on %s: %s", where.c_str(), failure().c_str());
